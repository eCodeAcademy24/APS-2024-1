...
public class Main {
    ...
    //        Банката решила дека сака да направи измена, односно да го отстрани најмалку важниот клиент од Golden листата и да го
    //        стави нa крај на Normal листата.
    ...
    public static void bank(SLL<Client> normal, SLL<Client> golden) {
        ...
        SLLNode<Client> goldenNode = golden.getFirst().succ;

//        int min = leastImportantGolden.element.calculateImportance();

        while (goldenNode != null) {
//            int importance = goldenNode.element.calculateImportance();
            if (goldenNode.element.calculateImportance() < leastImportantGolden.element.calculateImportance()) {
                leastImportantGolden = goldenNode;
            }
        ...
        SLLNode<Client> normalNode = normal.getFirst().succ;

        while (normalNode != null) {
            if (normalNode.element.calculateImportance() > mostImportantNormal.element.calculateImportance()) {
                mostImportantNormal = normalNode;
            }
