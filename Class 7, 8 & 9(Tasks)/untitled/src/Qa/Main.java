...
public class Main {
    ...
    public static void alterTeams(SLL devTeam, SLL qaTeam) {
        //prv cekor e da go najdeme najmladiot clen od qaTeam
        SLLNode najmlad = qaTeam.getFirst();
        SLLNode tmp = qaTeam.getFirst().succ;

        while (tmp != null) {
            if (najmlad.age >= tmp.age) {
                najmlad = tmp;
            }
        ...
        //vtor cekor e da ja najdeme sredinata na devTeam i da go vmetneme najmlad
        SLLNode tmp2 = devTeam.getFirst();
        for (int i = 0; i < (devTeam.length() + 1) / 2; i++) {
            tmp2 = tmp2.succ;
        }

        devTeam.insertBefore(najmlad.id, najmlad.age, tmp2);
        // tret cekor da go otstrani me najmlad od qaTeam
